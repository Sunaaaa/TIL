...
makemigrations로 만든 설계도를 실제 데이터 베이스 (sqlite3) 에 반영한다.
...
Ctrl + Shift + p를 눌러 sqlite 데이터 베이스를 Open할 프로젝트를 선택한다.
...
기본 생성자를 이용해, 각 컬럼마다 데이터를 지정한다.

생성자를 이용해 인자로 컬럼의 데이터를 지정한다.

ORM의 create() 함수 이용

1과 2번 방법의 경우, 실제 데이터 베이스에 반영되기 위해 save() 함수를 반.드.시 호출해야 한다.
...
<Article: [2] : second>, <Article: [3] : third>
...
서버를 실행하고 http://127.0.0.1:8000/admin로 진입한다.
